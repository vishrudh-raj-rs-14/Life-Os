...
import type { Cadence, Habit, Log } from "@/types";
...
const AREA_LABELS: Record<string, string> = {
  career: "Career", health: "Health", mind: "Mind", wealth: "Wealth",
  craft: "Craft", relationships: "Relationships", lifestyle: "Lifestyle",
};

const KIND_GLYPH: Record<Habit["kind"], string> = {
  binary: "·", count: "#", duration: "⏱", checklist: "☰",
};

function weeklyStats(habit: Habit, logs: Log[]) {
...
export function GoalsList() {
  ...
  const rows = useMemo(() => {
    ...
    return habits.map((h) => ({
      habit: h,
      stats: weeklyStats(h, logs),
      todayStatus: habitDoneToday(h, logs, today),
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <h2 className="serif text-2xl text-[var(--ink-1)]">All Goals</h2>
        </div>
        <Link href="/goals/new">
      ...
      {rows.length === 0 && (
        <div className="os-block p-8 text-center">
          ...
          <Link href="/goals/new">
            <Button><Plus size={14} /> First goal</Button>
          </Link>
        </div>
      )}
      ...
      {byArea.map(([area, areaRows]) => (
        <div key={area} className="space-y-1.5">
          ...
          {areaRows.map(({ habit, stats, todayStatus, dueToday }) => {
            ...
            const displayTarget =
              habit.weeklyTarget ?? cadenceWeeklyDefault(habit.cadence, habit.customDays);
            const displayPct = displayTarget > 0 ? Math.min(1, stats.done / displayTarget) : 0;
            return (
                ...
                <div className={cn(
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium truncate text-[var(--ink-1)]">{habit.title}</div>
